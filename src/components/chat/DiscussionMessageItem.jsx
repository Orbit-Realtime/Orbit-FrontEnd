...
function DiscussionMessageItem({ dm, isMine }) {
  ...
  return (
    <div className="flex flex-col items-start gap-0.5 min-w-0 max-w-full">
      <span className="text-xs font-medium text-orbit-muted ml-1">{dm.senderNickname}</span>
      <div className="max-w-[85%] min-w-0 overflow-hidden whitespace-pre-wrap [overflow-wrap:anywhere] bg-orbit-surface2 rounded-xl rounded-tl-sm px-3 py-2 text-white text-sm">
        <MessageContentRenderer content={dm.content} />
      </div>
