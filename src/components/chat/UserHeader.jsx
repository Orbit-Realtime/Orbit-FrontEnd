    ...
    <>
      <div className="flex items-center gap-2 px-4 py-2 border-b border-orbit-border flex-shrink-0">
        <div className="flex-1 min-w-0">
          {isEditingNickname ? (
            <input
              ...
              className="w-full bg-orbit-elevated text-white text-sm font-bold px-2 py-0.5 rounded outline-none border border-orbit-border-strong"
            />
          ) : (
            <button
              onClick={startEditNickname}
              className="font-bold text-orbit-text hover:text-orbit-secondary transition-colors text-left truncate w-full"
              title="클릭하여 닉네임 변경"
            >
              {auth?.nickname}
            </button>
        ...
        <button
          ...
          title="비밀번호 변경"
          className="flex-shrink-0 p-1.5 rounded-lg text-orbit-muted hover:text-orbit-text hover:bg-orbit-surface2 transition-colors"
        >
